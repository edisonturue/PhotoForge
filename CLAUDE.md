...
> 不管用户怎么描述需求，AI 必须在动手之前先执行完整推演。
> 用户说的每句话都是"冰山一角"，AI 的职责是把水面下的部分补全，并推荐最优实现路径。

### 第一步：意图展开

收到任何功能/改动需求后，在写任何代码之前，先输出推演表：

```
【意图推演】
用户意图：[用一句话概括用户想要什么]
功能类型：实体管理型 / 工具操作型 / 系统流程型 / 展示视图型

必须同时完成的事项（不可遗漏）：
1. [主功能本身]
2. [关联的已有功能需要适配的地方]
3. [数据层变更（types.ts / store.ts / constants.ts）]
4. [UI 层变更（新增组件 / 修改已有组件）]
5. [国际化（i18n 中英文）]
6. [快捷键（如果适用）]
7. [交互状态（hover/active/focus/disabled/selected/loading）]
8. [过渡动画]
9. [测试覆盖]
10. [边界情况/错误处理]
...
1. **实体穿透**：涉及哪些实体（Photo/Preset/Collection/Settings）？→ 检查 CRUD 完整性 → 检查该实体在所有已有功能中是否需要联动。
2. **场景穿透**：出现在哪些用户旅程中？→ 对照「需求发现汇总 - 场景驱动发现」→ 是否打断了已有旅程。
3. **接口穿透**：涉及哪些层？→ IPC handler → React 组件 → 共享类型 → i18n → 测试，每层列出新增/修改。

推演完成后自检 7 个问题，任何一个答"否"必须补充到任务列表：

1. 用户是否会期望在搜索结果中看到这个新内容？
2. 用户是否会期望在筛选条件中找到这个新内容？
3. 是否有"空状态"？（列表为空时显示什么？）
4. 是否有"加载状态"？
5. 是否有"错误状态"？（操作失败怎么办？）
6. 增删改查是否都支持？还是只有 Create 和 Read？
7. 是否有快捷键？是否需要更新快捷键文档？

### 第二步：方案推荐（不可跳过）

意图展开完成后，AI 必须提出 **2~3 个可行方案** 并推荐最优方案。禁止直接选第一个想到的方案就开始写代码。
...
推荐方案：[X]，原因：[结合项目现状的具体理由]

不推荐的方案说明：
- 方案 [Y] 不推荐，因为 [具体原因，如"与现有 store.ts 的数据模型冲突"]
```

#### 方案推荐时的决策框架

推荐方案时必须综合评估以下维度，不可只看"能不能实现"：

| 维度 | 评估要点 |
|------|---------|
| **一致性** | 是否与项目已有的架构模式一致？（如 IPC 通信模式、组件拆分粒度、状态管理方式） |
| **侵入性** | 需要改动多少已有文件？改动越大，回归风险越高 |
| **可逆性** | 如果方案不对，是否容易回退？优先选择可逆性高的方案 |
| **用户感知** | 用户能不能立即感受到价值？优先选择"做了就能用"而非"需要先做大量基础设施" |
| **依赖链** | 是否需要先完成其他前置功能？如果是，要明确指出前置依赖 |
| **数据兼容** | 是否需要迁移 library.json 的数据结构？如果是，需要提供向后兼容方案 |
| **竞品参考** | 竞品（Lightroom / darktable / Apple Photos）是怎么做的？为什么？ |

#### 推荐的表达方式

AI 推荐方案时的语气应该是**顾问式的**，不是执行式的：

```
❌ "我将使用方案 A 来实现。"
✅ "我建议用方案 A。原因是现有 store.ts 已经用了类似的 X 模式，改动最小；
    方案 B 虽然更灵活，但需要重构整个数据层，目前阶段性价比不高。你觉得呢？"
```

当存在明显最优解时可以直接推荐，但必须说明理由；当方案各有利弊时，必须明确列出权衡点让用户决定。

### 第三步：风险预警

在推荐方案后，AI 必须列出此功能的潜在风险：

```
【风险预警】
- 🔴 高风险：[可能导致功能失败或数据丢失的问题]
- 🟡 中风险：[可能导致体验不佳或需要返工的问题]
- 🟢 低风险：[小问题，但值得知道]
- ⚪ 已知取舍：[做了 A 就不能做 B 的情况，说明为什么选 A]
```
...
| 功能域 | 说明 | 覆盖程度 |
|--------|------|---------|
| 文件管理 | 导入/导出/去重/引用管理 | ✅ 较完整 |
| 图像浏览 | 网格/详情/对比/缩略图 | ✅ 基本完整 |
| 图像编辑/预设 | 预设创建/应用/管理 | ⚠️ 缺参数编辑 |
| 组织/分类 | 相册/收藏/标签/颜色标签 | ⚠️ 缺智能相册 |
| 元数据 | EXIF读取/展示/搜索 | ⚠️ 元数据编辑缺失 |
| 性能/缓存 | 缩略图缓存/RAW转换缓存 | ✅ 有 |
| 搜索/筛选 | 文本搜索/多条件筛选 | ⚠️ 搜索弱 |
| 导入导出流程 | 批量导入/导出配置 | ✅ 较完整 |
| 历史与撤销 | 撤销/重做/编辑历史 | ❌ 完全缺失 |
...
| 功能 | 类型 | 发现来源 | 实现参考 |
|------|------|---------|---------|
| 撤销/重做 | 系统/流程型 | 竞品(LR+darktable都有) | darktable 历史栈 |
| 全局搜索 | 工具/操作型 | 实体驱动+竞品 | darktable 搜索实现 |
| 预设搜索 | 工具/操作型 | 实体驱动+竞品(LR) | LR 预设面板搜索框 |
| 预设参数编辑 | 实体管理型 | 生命周期缺口+竞品 | LR 滑块 UX |
| 预设收藏 | 实体驱动+竞品 | 竞品(LR) | LR Favorites |
| 编辑历史 | 系统/流程型 | 竞品(LR+darktable) | darktable history |

**中优先（显著提升体验）**

| 功能 | 类型 | 发现来源 | 实现参考 |
|------|------|---------|---------|
| 统计视图 | 展示/视图型 | 实体驱动+竞品(Apple) | Apple Photos 统计 |
| 按日期分组 | 展示/视图型 | 场景驱动+竞品(Apple) | Apple Photos 时刻 |
| 最近导入入口 | 工具/操作型 | 场景驱动 | — |
| 智能相册 | 实体管理型 | 场景驱动+竞品 | Apple Photos 条件填充 |
| 导出命名规则 | 工具/操作型 | 场景驱动+竞品(LR) | LR 命名模板 |
| 相册编辑 | 实体管理型 | 生命周期缺口 | — |
| 直方图 | 展示/视图型 | 竞品(LR+darktable) | darktable 直方图模块 |
| 导出后打开文件夹 | 工具/操作型 | 场景驱动 | — |
