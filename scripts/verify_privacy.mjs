...
// Check personal workflow files excluded
const personalFiles = ['CLAUDE.md', 'AI_COLLABORATION_GUIDE.md', 'AGENTS.md'];
const trackedPersonal = personalFiles.filter(f => files.includes(f));
trackedPersonal.length === 0 ? ok('个人 AI 工作流文件 = 不在 Git 中') : fail(`个人文件仍被跟踪: ${trackedPersonal.join(', ')}`);

const oldPlans = ['DEVELOPMENT_PLAN.md', 'FIX_PLAN.md', 'FIX_PLAN_V2.md', 'SKILL_PLAN_requirements_expander.md'];
const trackedPlans = oldPlans.filter(f => files.includes(f));
trackedPlans.length === 0 ? ok('已废弃计划文件 = 不在 Git 中') : fail(`旧计划文件仍被跟踪: ${trackedPlans.join(', ')}`);

// Check gitignore has .codex/ etc
const gitignore = readFileSync(resolve(root, '.gitignore'), 'utf-8');
gitignore.includes('.codex/') ? ok('.gitignore 已排除 .codex/ 目录') : fail('.gitignore 缺少 .codex/');
gitignore.includes('release/') ? ok('.gitignore 已排除 release/ 构建产物') : fail('.gitignore 缺少 release/');
gitignore.includes('dist/') ? ok('.gitignore 已排除 dist/ 编译输出') : fail('.gitignore 缺少 dist/');
gitignore.includes('node_modules') ? ok('.gitignore 已排除 node_modules') : fail('.gitignore 缺少 node_modules/');
...
if (pass === total) {
  ...
  console.log('   → 不会。所有文件操作在 ~/Pictures/PhotoForge_Library/ 内部循环。\n');
